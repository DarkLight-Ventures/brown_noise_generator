...
fn main() {
    ...
    let brown_noise_samples = generate_brown_noise(duration_secs, sample_rate);
...
fn apply_warble_effect(samples: &[i16], lfo_frequency: f64, sample_rate: u32, depth: f32) -> Vec<i16> {
    let mut warbled_samples = vec![0i16; samples.len()];
    let lfo_increment = 2.0 * PI * lfo_frequency / (sample_rate as f64);
...
fn generate_brown_noise(duration_secs: u32, sample_rate: u32) -> Vec<i16> {
